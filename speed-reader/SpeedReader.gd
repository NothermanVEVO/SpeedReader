...
var _words_per_minute : float = 600
var _currently_words_per_minute : float = 600
var _current_word : String = ""

var _is_paused : bool = false

func _ready() -> void:
	#set_text("Oi, tudo bem com você? Estou testando para ver se está tudo funcionando como o planejado. E então, foi?")
...
func play() -> void:
	while not _is_paused:
		...
		if _currently_word_idx >= _words.size():
			return
		_current_word = _words[_currently_word_idx].strip_edges()
		queue_redraw()
